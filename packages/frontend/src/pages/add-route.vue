<template>
  <div class="add-route">
    <header class="add-route__header">
      <NuxtLink to="/dashboard" class="add-route__back" :aria-label="t('nav.dashboard')">
        ← Back
      </NuxtLink>
    ...
    <form class="add-route__form" @submit.prevent="handleSubmit">
      ...
      <div class="add-route__checkbox">
        <input id="flexible" v-model="form.flexibleDates" type="checkbox" data-cy="checkbox-flexible" />
        <label for="flexible">{{ t('addRoute.flexibleDates') }}</label>
      </div>

      <div v-if="error" class="add-route__error">{{ error }}</div>

      <button type="submit" class="add-route__submit" :disabled="isSubmitting" data-cy="btn-submit">
...
const form = ref({ origin: '', destination: '', flexibleDates: false })
...
async function handleSubmit() {
  ...
  const route = await routesStore.createRoute({
...
.add-route__back:hover { color: var(--text-primary); }
...
.add-route__input:focus { border-color: var(--border-focus); }
...
.add-route__submit:hover:not(:disabled) { background-color: var(--accent-hover); }
.add-route__submit:disabled { opacity: 0.5; cursor: not-allowed; }
</style>
