<template>
  <article class="alert-card" :class="`alert-card--${alert.level.toLowerCase()}`">
    ...
    <footer class="alert-card__footer">
      ...
      <span class="alert-card__time">{{ formatDate(alert.createdAt) }}</span>
    </footer>
  </article>
</template>
...
const props = defineProps<IAlertCardProps>()
const { t } = useI18n()
...
.alert-card__badge[data-level='INSANE'] {
  background-color: var(--badge-insane-bg);
  color: var(--badge-insane-text);
}

.alert-card__badge[data-level='GOOD'] {
...
.alert-card__price-value--highlight {
  color: var(--accent);
}
